			...
			connection.query(sql_player_data, [round], function(err2, results2) {
				...
				connection.query(sql_net_expense,function(err3,reply3){
					...
					connection.query(sql_prem_spent,function(err4,reply4){
						...
						redisClient.zrevrange('UFLTeamRanks', 0, 1500000, "withscores", function(err5, reply5) {
	...
	connection.query(sql_round_count, function(err, results){
		...
		connection.query(sql_player_data,[groupID],function(err2,result2){
			...
			connection.query(sql_group_data,[groupID],function(err1,res1){
				...
				redisClient.zrevrange('uflTeamRanks', 0, 1500000, "withscores", function(err3, reply3) {
					if(err3) throw err3;
...
app.post('/gamemaster/assignPlayers', function(req,res){
	...
	redisClient.get('currentRound', function(err20, groupIdResp) {
		...
		redisClient.hget("uflteam1", 'yourBid', function(err,result){
			if(err) throw err;
		...
		redisClient.hget("uflteam1",'premLeft',function(err,result){
			if(err) throw err;
		...
		redisClient.hget("uflteam2", 'yourBid', function(err,result){
			if(err) throw err;
			let sql_player_price = "UPDATE Players SET price = ? WHERE team_id = ? AND group_id = ?";
		...
		//PREMIUM INTO DATABASE
		redisClient.hget("uflteam2",'premLeft',function(err,result){
			if(err) throw err;
			let sql_prem_left = "UPDATE Bidders SET premium_left=? WHERE team_id=?";
			connection.query(sql_prem_left,[result,2],function(err2,res2){});
		});

		redisClient.hget("uflteam3", 'yourBid', function(err,result){
			if(err) throw err;
			let sql_player_price = "UPDATE Players SET price = ? WHERE team_id = ? AND group_id = ?";
		...
		//PREMIUM INTO DATABASE
		redisClient.hget("uflteam3",'premLeft',function(err,result){
			if(err) throw err;
			let sql_prem_left = "UPDATE Bidders SET premium_left=? WHERE team_id=?";
			connection.query(sql_prem_left,[result,3],function(err2,res2){});
		});

		redisClient.hget("uflteam4", 'yourBid', function(err,result){
			if(err) throw err;
			let sql_player_price = "UPDATE Players SET price = ? WHERE team_id = ? AND group_id = ?";
		...
		//PREMIUM INTO DATABASE
		redisClient.hget("uflteam4",'premLeft',function(err,result){
			if(err) throw err;
			let sql_prem_left = "UPDATE Bidders SET premium_left=? WHERE team_id=?";
			connection.query(sql_prem_left,[result,4],function(err2,res2){});
		});

		redisClient.hget("uflteam5", 'yourBid', function(err,result){
			if(err) throw err;
			let sql_player_price = "UPDATE Players SET price = ? WHERE team_id = ? AND group_id = ?";
		...
		//PREMIUM INTO DATABASE
		redisClient.hget("uflteam5",'premLeft',function(err,result){
			if(err) throw err;
			let sql_prem_left = "UPDATE Bidders SET premium_left=? WHERE team_id=?";
			connection.query(sql_prem_left,[result,5],function(err2,res2){});
		});

		redisClient.hget("uflteam6", 'yourBid', function(err,result){
			if(err) throw err;
			let sql_player_price = "UPDATE Players SET price = ? WHERE team_id = ? AND group_id = ?";
		...
		//PREMIUM INTO DATABASE
		redisClient.hget("uflteam6",'premLeft',function(err,result){
			if(err) throw err;
			let sql_prem_left = "UPDATE Bidders SET premium_left=? WHERE team_id=?";
			connection.query(sql_prem_left,[result,6],function(err2,res2){});
		});

		redisClient.zrevrange('uflTeamRanks', 0, 15000000, function(err2, rankingArray) {
			if(err2) throw err2;
			...
			connection.query(sql_group_base_bid, [nextRound], function(err50, reply50){
				...
				for(var i=0; i<6; i++) {
					...
					redisClient.zadd('uflTeamRanks', baseBidForNextRound + i * 1000, teamCodeInSortedSet);
...
app.post('/teamprofile', function(req, res) {
	...
	connection.query(sql, [teamID], function(err, result) {
		...
		redisClient.zscore('uflTeamRanks', key, function(err1, reply){
			// console.log("Team Rank", reply);
			var totalSpent = parseInt(result[0].points_spent) + parseInt(reply);

			redisClient.hgetall("uflteam" + teamID, function(err, object) {
				...
				connection.query(sql_teams, [teamID], function(err2, result2) {
					...
					connection.query(sql_players, [currRound], function(err3, result3) {
						...
						if(teamObject.bidFlag == 1){
							...
							var disableBtn = true;
							redisClient.zrevrange('uflTeamRanks', 0, 15000000, "withscores", function(err4, reply4) {
								res.render('bidding.ejs', {
									currentRound: currRound,
									group_object: grp_obj,
									team_object: team_obj,
...
io.on('connection', function(client) {
			...
			redisClient.get('maxBid', function(err1, reply1) {
				...
				if(currBid < maxBid) {
					...
					redisClient.hgetall(team, function(err5, teamObject) {
						...
						redisClient.zadd('uflTeamRanks', currBid, team);
						redisClient.hset(team, 'yourBid', currBid);
						redisClient.zrevrank('uflTeamRanks', team, function(err3, reply3) {
							if(err3) throw err3;
						...
						redisClient.zrevrange('uflTeamRanks', 0, currBid, "withscores", function(err2, reply2) {
							if(err2) throw err2;
	...
	client.on('login', function(data) {
		...
		let key = "uflteam" + teamID;

		redisClient.zadd('uflTeamRanks', firstBid, key);
		redisClient.zrevrange('uflTeamRanks', 0, 1500000, "withscores", function(err, reply) {
